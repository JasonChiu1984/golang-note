...
> 教材版本：`v1.0.17`
> 教材基準：`Go 1.26.3`
> 這次更新重點：補齊 startup configuration contract，避免部署錯誤設定被 silent fallback 掩蓋。
...
| `production-api-worker` | 練習 HTTP API、startup config、strict request decoding、transaction、context-aware retry、request timeout contract、queue shutdown safety、observability、panic recovery、graceful shutdown、Docker Compose | 完成第 5、7、9、11 章後 | `cd production-api-worker && go test ./...` |
...
| Startup config 合約 | `cd production-api-worker && go test ./internal/config -count=1` | 驗證 `PORT`、`QUEUE_SIZE`、`WORKERS` 預設值、合法 env 與錯誤設定 fail-fast 行為 |

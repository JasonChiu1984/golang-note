...
Go ReleaseNote contract gate 由 `node scripts/check-go-release-notes-contract.mjs` 固定：`scripts/generate-go-release-notes.mjs`、`ReleaseNote/` 與 `docs/ReleaseNote/` 必須同步保留 Go 1.1-1.26 專業報告、官方來源、`Patch Revisions`、支援狀態圖與 Go 1.26.5 / Go 1.25.12 最新 patch 訊號，避免版本升級教材只剩人工檢查。Go ReleaseNote freshness evidence 由 `node scripts/check-go-release-notes-freshness-contract.mjs` 固定 official Go Release History verified 時間、ReleaseNote index 來源證據與最新 patch baseline，避免 token 沒變時漏掉本輪官方查核紀錄。

Release version consistency contract gate 由 `node scripts/check-release-version-consistency-contract.mjs` 固定：`VERSION`、`CHANGELOG.md`、README、API contract、OpenAPI、章節、整合視覺課程、`docs/index.html`、Makefile 與 GitHub Actions 必須標示同一個目前 release 版本，避免 commit、tag、Pages 首頁或 contract 文件顯示不同版本。
...
Release artifact metadata consistency contract gate 由 `node scripts/check-release-artifact-metadata-contract.mjs` 固定：同 timestamp 的審查報告、內容需要更新的部分與更新資料必須使用一致的版本、完整日期時間、本輪主題、來源引用與 artifact reference，避免 release artifact 檔案存在但 metadata 無法追溯。
Release artifact continuity index contract gate 由 `node scripts/check-release-artifact-continuity-contract.mjs` 固定：近期 release artifacts 必須保留版本 sequence、timestamp 配對、三件套對應與 update record 自我引用，避免最新紀錄存在但歷史鏈斷裂。

Release publish reconciliation contract gate 由 `node scripts/check-release-publish-reconciliation-contract.mjs` 固定：當 remote release 已建立、但 final release-record amend 或 tag 更新仍 local-only 時，更新紀錄必須保留 `HEAD`、`origin/main`、`tag^{}`、`force-with-lease` 與 recovery command；當 blocked-push recovery 完成後與 Release publish recovery continuation，也必須新增 finalization 紀錄保存成功推送輸出與 Release publish recovery continuation。這避免下一輪 release 誤把 local-only final amend 當成已完整發布、誤以為 release 仍 blocked，或在修復時覆蓋遠端 release commit。
...
Dependency audit evidence freshness contract gate 由 `node scripts/check-dependency-audit-evidence-contract.mjs` 固定：每次 release 都需保留 root module 與 `production-api-worker` 的 dependency audit evidence 與 release evidence retention；若 module proxy / vulnerability database 無法連線，更新紀錄必須標示待補掃描與原因，不可把離線限制誤標為 `govulncheck` 通過。
...
SLO incident response governance contract gate 由 `node scripts/check-slo-incident-response-governance-contract.mjs` 與 `cd production-api-worker && make slo-incident-response-governance-check` 固定：正式事故需保留 availability SLO、error budget policy、incident commander、escalation policy、mitigation decision、customer impact note 與 postmortem action owner。它把 observability 與 Alertmanager routing 推進到可審核的事故治理鏈，避免只知道告警與 rollback，卻不知道是否 breach SLO、誰負責決策、如何溝通客戶影響及後續 action 由誰關閉。

Structured log schema governance contract gate 由 `node scripts/check-structured-log-schema-governance-contract.mjs` 與 `cd production-api-worker && make structured-log-schema-governance-check` 固定：正式 log evidence 需保留 `request_id`、`trace_id`、`severity`、`error_code`、`route`、log redaction policy、log retention owner 與 audit log review cadence。它把 `slog` 教學推進到可審核的 log schema governance，避免事故後 log 欄位命名、遮蔽或保留責任漂移。
...
Production incident response 不應只停在 metrics、trace 或 rollback 指令。`node scripts/check-slo-incident-response-governance-contract.mjs` 與 `make slo-incident-response-governance-check` 固定 availability SLO、error budget policy、incident commander、escalation policy、mitigation decision、customer impact note 與 postmortem action owner，讓 SLO breach review 與 incident evidence retention 可被 release gate 回歸。

...
Release publish reconciliation contract gate 由 `node scripts/check-release-publish-reconciliation-contract.mjs` 固定：當 remote release 已建立、但 final release-record amend 或 tag 更新仍 local-only 時，更新紀錄必須保留 `HEAD`、`origin/main`、`tag^{}`、`force-with-lease` 與 recovery command。這避免下一輪 release 誤把 local-only final amend 當成已完整發布，或在修復時覆蓋遠端 release commit。

...
大型專案的 incident response 也應視為架構邊界。SLO incident response governance contract gate 由 `node scripts/check-slo-incident-response-governance-contract.mjs` 固定 availability SLO、error budget policy、incident commander、escalation policy、mitigation decision、customer impact note 與 postmortem action owner，避免 production 事故只留下 log 與 rollback 指令，卻沒有可追溯的決策責任。

Structured log schema governance contract gate 由 `node scripts/check-structured-log-schema-governance-contract.mjs` 固定 `request_id`、`trace_id`、`severity`、`error_code`、`route`、log redaction policy、log retention owner 與 audit log review cadence。它讓 incident review 不只依賴 request id，而是保留可查詢、可遮蔽、可保存且有人負責的 log evidence。
...
| Operational runbook scope freshness contract gate | `node scripts/check-operational-runbook-scope-contract.mjs`、runbook metadata、API contract scope coverage、Docs publishing contract gate、Release artifact chain contract gate | runbook 首段日期、版本與 scope 落後於近期 release governance gate |
...
| Contract gate inventory | `node scripts/check-contract-gate-inventory-contract.mjs`、58 個 root contract checker、GitHub Actions 呼叫清單 | 新增 checker 後沒有進入 CI、Makefile 或教材入口，導致 release gate 漂移 |
...
| Go ReleaseNote contract gate | `node scripts/check-go-release-notes-contract.mjs`、Go 1.1-1.26 報告、根目錄與 Pages 同步、最新 patch 訊號 | ReleaseNote 產生器或 Pages 同步後遺失官方來源、支援狀態、必要報告區塊或 Go 1.26.5 / Go 1.25.12 patch revisions |
| Release version consistency contract gate | `node scripts/check-release-version-consistency-contract.mjs`、VERSION、CHANGELOG、README、API contract、OpenAPI、章節、整合視覺課程、docs/index | 發版後 commit、tag、Pages 首頁或 contract 文件顯示不同版本 |
...
| Release artifact metadata consistency contract gate | `node scripts/check-release-artifact-metadata-contract.mjs`、審查報告、內容需要更新的部分、更新資料、timestamp、版本、主題 | 發版 artifact 檔案存在但 metadata 或交叉引用漂移 |
| Release artifact continuity index contract gate | `node scripts/check-release-artifact-continuity-contract.mjs`、release artifacts、version sequence、timestamp pairing、update record self reference | 最新 release artifact 存在但歷史三件套或版本連續性斷裂 |
| Release publish reconciliation contract gate | `node scripts/check-release-publish-reconciliation-contract.mjs`、`HEAD`、`origin/main`、`tag^{}`、`force-with-lease`、recovery command、Release publish recovery continuation | remote release 已建立但 final release-record amend / tag 更新仍 local-only 時，下一輪發版可能誤判發布狀態或覆蓋遠端 release commit |
...
| Dependency audit evidence freshness contract gate | `node scripts/check-dependency-audit-evidence-contract.mjs`、release evidence retention、root / production module audit coverage | release 更新紀錄把 module proxy / vulnerability database 離線限制誤寫成 `govulncheck` 通過 |
...
| Contract gate inventory | `node scripts/check-contract-gate-inventory-contract.mjs` 固定 58 個 root contract checker 都被 GitHub Actions 呼叫，避免 checker 只存在於 repo 沒有進入 release gate |
...
| Release version consistency contract gate | `node scripts/check-release-version-consistency-contract.mjs` 固定 VERSION、CHANGELOG、README、API contract、OpenAPI、章節、整合視覺課程、docs/index、Makefile 與 CI 的目前版本一致性 |
...
| Release artifact metadata consistency contract gate | `node scripts/check-release-artifact-metadata-contract.mjs` 固定三份 release artifacts 的 timestamp、版本、完整日期時間、本輪主題與交叉引用 |
| Release artifact continuity index contract gate | `node scripts/check-release-artifact-continuity-contract.mjs` 固定近期 release artifacts 的版本 sequence、timestamp 配對、三件套對應與 update record 自我引用 |
| Release publish reconciliation contract gate | `node scripts/check-release-publish-reconciliation-contract.mjs` 固定 remote-created / local-final-amended release 與 blocked-push recovery finalization / Release publish recovery continuation 的 `HEAD`、`origin/main`、`tag^{}`、`force-with-lease`、recovery command 與成功推送輸出與 Release publish recovery continuation |
...
| Dependency audit evidence freshness contract gate | `node scripts/check-dependency-audit-evidence-contract.mjs` 固定 release evidence retention、root / production module dependency audit coverage，以及離線待補掃描語意 |

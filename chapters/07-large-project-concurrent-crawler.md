...
| Release publish reconciliation contract gate | `node scripts/check-release-publish-reconciliation-contract.mjs`、`HEAD`、`origin/main`、`tag^{}`、`force-with-lease`、recovery command | remote release 已建立但 final release-record amend / tag 更新仍 local-only 時，下一輪發版可能誤判發布狀態或覆蓋遠端 release commit |

...
Production 測試也要覆蓋事故治理文件面。SLO incident response governance contract gate 由 `node scripts/check-slo-incident-response-governance-contract.mjs` 固定 availability SLO、error budget policy、incident commander、escalation policy、mitigation decision、customer impact note 與 postmortem action owner；這類 checker 不測 runtime branch，而是防止 runbook、API contract、CI 與教程入口漏掉 postmortem action owner 或 escalation policy。

Structured log schema governance contract gate 由 `node scripts/check-structured-log-schema-governance-contract.mjs` 固定 `request_id`、`trace_id`、`severity`、`error_code`、`route`、log redaction policy、log retention owner 與 audit log review cadence；這類 checker 不測 logging backend，而是防止文件、OpenAPI、runbook、CI 與教程入口漏掉可審核的 log schema。
...
Go ReleaseNote contract gate 固定 generated report chain，不讓 `scripts/generate-go-release-notes.mjs`、`ReleaseNote/` 或 `docs/ReleaseNote/` 在重產或發布同步後遺失 Go 1.1-1.26 必要報告區塊、官方來源、支援狀態、Patch Revisions 或 Go 1.26.5 / Go 1.25.12 最新 patch 訊號。Go ReleaseNote freshness evidence 由 `node scripts/check-go-release-notes-freshness-contract.mjs` 固定 official Go Release History verified 時間、ReleaseNote index 來源證據與最新 patch baseline。

Release version consistency contract gate 固定目前版本同步，不讓 `VERSION`、`CHANGELOG.md`、README、API contract、OpenAPI、章節、整合視覺課程、`docs/index.html`、Makefile 或 GitHub Actions 在 release 後顯示不同版本。它由 `node scripts/check-release-version-consistency-contract.mjs` 與 `cd production-api-worker && make release-version-consistency-check` 固定。
...
Release artifact metadata consistency contract gate 固定同 timestamp 的三份 release artifacts，不讓審查報告、內容需要更新的部分或更新資料在版本、完整日期時間、本輪主題、來源引用與 artifact reference 上漂移。它由 `node scripts/check-release-artifact-metadata-contract.mjs` 與 `cd production-api-worker && make release-artifact-metadata-check` 固定。

Release publish reconciliation contract gate 固定發布收斂證據：remote release 已建立但 final release-record amend / tag 更新仍 local-only 時，更新紀錄必須保留 `HEAD`、`origin/main`、`tag^{}`、`force-with-lease` 與 recovery command；blocked-push recovery 完成後與 Release publish recovery continuation，也要保留成功推送輸出與 Release publish recovery continuation 與 finalization artifact。它由 `node scripts/check-release-publish-reconciliation-contract.mjs` 檢查，避免下一輪 release 把 local-only final amend 誤判成已完整發布，或把已補推的 release 誤判成仍 blocked。
...
Dependency audit evidence freshness contract gate 固定 release evidence retention。它要求每次 release 都能追溯 root module 與 production module 的 dependency audit evidence；`module proxy / vulnerability database` 離線只能寫成待補掃描，不可把沒有取得資料庫的 `govulncheck ./...` 包裝成通過。
...
| Structured log schema governance contract gate | `node scripts/check-structured-log-schema-governance-contract.mjs` | 固定 `request_id`、`trace_id`、`severity`、`error_code`、`route`、log redaction policy、log retention owner 與 audit log review cadence |
...
| Contract gate inventory | `node scripts/check-contract-gate-inventory-contract.mjs && cd production-api-worker && make contract-gate-inventory-check` | 固定 58 個 root contract checker 都被 GitHub Actions 呼叫，避免 checker 只存在於 repo 沒有進入 release gate |
...
| Release artifact metadata consistency contract gate | `node scripts/check-release-artifact-metadata-contract.mjs && cd production-api-worker && make release-artifact-metadata-check` | 固定三份 release artifacts 的 timestamp、版本、完整日期時間、本輪主題與交叉引用 |
| Release artifact continuity index contract gate | `node scripts/check-release-artifact-continuity-contract.mjs && cd production-api-worker && make release-artifact-continuity-check` | 固定 release artifact 版本連續、timestamp 配對與 update record 自我引用 |
| Release publish reconciliation contract gate | `node scripts/check-release-publish-reconciliation-contract.mjs && cd production-api-worker && make release-publish-reconciliation-check` | 固定 remote-created / local-final-amended release 與 blocked-push recovery finalization / Release publish recovery continuation 的 `HEAD`、`origin/main`、`tag^{}`、`force-with-lease`、recovery command 與成功推送輸出與 Release publish recovery continuation |
...
| Dependency audit evidence freshness contract gate | `node scripts/check-dependency-audit-evidence-contract.mjs && cd production-api-worker && make dependency-audit-evidence-check` | 固定 release evidence retention、root / production module audit coverage 與待補掃描語意 |
...
| Alertmanager routing governance contract gate | `node scripts/check-alertmanager-routing-contract.mjs` | 確認 Alertmanager route、receiver owner、escalation owner、silence policy、notification evidence、Prometheus alerting target 與 Compose service 一起進入 release gate |
| Operational runbook scope freshness contract gate | `node scripts/check-operational-runbook-scope-contract.mjs` | 確認 runbook metadata、完整日期時間與近期 release governance scope 沒有漂移 |
...
| Contract gate inventory | `node scripts/check-contract-gate-inventory-contract.mjs` | 確認 58 個 root contract checker 全部被 GitHub Actions 呼叫 |
...
| Go ReleaseNote freshness evidence | `node scripts/check-go-release-notes-freshness-contract.mjs` | 確認官方查核時間、ReleaseNote index 來源證據與最新 patch baseline 沒有漂移 |
| Release version consistency contract gate | `node scripts/check-release-version-consistency-contract.mjs` | 確認 release 版本在核心文件、OpenAPI、章節、整合視覺課程與 CI 中一致 |
...
| Release artifact metadata consistency contract gate | `node scripts/check-release-artifact-metadata-contract.mjs` | 確認 release artifacts 的 metadata 與交叉引用一致 |
| Release publish reconciliation contract gate | `node scripts/check-release-publish-reconciliation-contract.mjs` | 確認 remote/local publish divergence、force-with-lease 與 recovery command 沒有漏件 |
...
| Dependency audit evidence freshness contract gate | `node scripts/check-dependency-audit-evidence-contract.mjs` | 確認 dependency audit evidence、release evidence retention 與待補掃描語意沒有漂移 |

...
| Release publish reconciliation contract gate | `node scripts/check-release-publish-reconciliation-contract.mjs` 必須固定 remote release 已建立但 final release-record amend / tag 更新仍 local-only 時的 `HEAD`、`origin/main`、`tag^{}`、`force-with-lease` 與 recovery command |

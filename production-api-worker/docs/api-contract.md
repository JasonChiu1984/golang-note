...
| Contract gate inventory | `node scripts/check-contract-gate-inventory-contract.mjs` 必須固定 53 個 root contract checker 都被 GitHub Actions 呼叫，且 Makefile、README、API contract、章節與整合視覺課程入口一致 |
...
| Release publish reconciliation contract gate | `node scripts/check-release-publish-reconciliation-contract.mjs` 必須固定 remote release 已建立但 final release-record amend / tag 更新仍 local-only 時的 `HEAD`、`origin/main`、`tag^{}`、`force-with-lease`、recovery command，以及 blocked-push recovery 成功推送輸出 |

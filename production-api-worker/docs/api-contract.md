...
> 版本：v1.0.94 ｜ 基準日期：2026-07-21 ｜ 適用範圍：local memory mode、Postgres + OTLP mode、OpenAPI contract、API contract scope coverage、Readiness lifecycle contract、Request decoding contract、Idempotency key contract、API latency metrics contract、Service transaction boundary contract、Panic recovery contract、Request correlation contract、API security contract、Rate limit contract、Shutdown signal contract、Trace shutdown contract、Trusted proxy client IP contract、CORS allowlist contract、Request body limit contract、HTTP server timeout contract、Startup configuration contract gate、Worker failure contract、Worker shutdown contract、Retry cancellation contract、Queue backpressure contract、DB pool contract gate、Migration Operation Contract、Prometheus config contract gate、Operational observability contract gate、Operational runbook scope freshness contract gate、Alertmanager routing governance contract gate、SLO incident response governance contract gate、Structured log schema governance contract gate、OTLP collector contract、OTLP export governance contract gate、Secret handling governance contract gate、Supply chain artifact governance contract gate、Platform promotion policy contract gate、Deployment controller config contract gate、CI quality gate contract、CI contract parity gate、Contract gate inventory、Docs publishing contract gate、Production workflow contract gate、Syntax flow SVG contract gate、Go ReleaseNote contract gate、Go ReleaseNote freshness evidence、Release version consistency contract gate、Release artifact chain contract gate、Release artifact metadata consistency contract gate、Release artifact continuity index contract gate、Release publish reconciliation contract gate、Dependency governance contract gate、Dependency audit evidence freshness contract gate、API edge security policy governance contract gate、Performance benchmark governance contract、Release rollback drill contract、Docker build contract、Compose runtime env contract、Compose smoke contract
...
API contract scope coverage 由 `node scripts/check-openapi-contract.mjs` 固定。新增或調整 release gate 時，除了更新表格與 OpenAPI description，也必須同步本文件首段適用範圍，避免 API contract header 漏列 Docs publishing、release artifact chain 或其他發布面 contract。
...
| API latency metrics contract | `api_request_duration_seconds` 必須以 `route`、`method`、`status` labels 輸出 HTTP latency histogram |
| API latency metrics gate | `node scripts/check-api-latency-metrics-contract.mjs` 必須固定 runtime histogram、metrics middleware、`TestAPILatencyMetricsContract`、README、章節、Makefile 與 CI 入口 |
| Service transaction boundary contract | `CreateJob` 必須使用 `sql.TxOptions{Isolation: sql.LevelReadCommitted}` 建立 job，commit 後才 enqueue，queue 滿載時需以 transaction 回寫 `failed` |
| Service transaction boundary gate | `node scripts/check-service-transaction-boundary-contract.mjs` 必須固定 `TestServiceTransactionBoundaryContract`、service 實作、README、章節、Makefile 與 CI 入口 |
...
| Worker shutdown contract | `Queue.Enqueue` 與 `Queue.ShutdownContext` 必須共用 mutex 保護 `closed` 狀態、channel send 與 close；shutdown 後新 enqueue 回 `worker.ErrClosed` |
| Worker shutdown gate | `node scripts/check-worker-shutdown-contract.mjs` 必須固定 `ErrClosed`、`TestEnqueueAfterShutdownReturnsClosedError`、`TestConcurrentEnqueueAndShutdownDoesNotPanic`、Makefile、README、章節與 CI 入口 |
...
| Structured log schema governance gate | `node scripts/check-structured-log-schema-governance-contract.mjs` 必須固定 structured log schema governance contract gate、log redaction policy、log retention owner 與 audit log review cadence |
...
| Startup configuration contract gate | `node scripts/check-startup-config-contract.mjs` 必須固定 `PORT`、`QUEUE_SIZE`、`WORKERS`、`OTEL_EXPORTER_OTLP_ENDPOINT`、config loader、Go tests、README、章節、Makefile 與 CI 入口 |
...
| API edge security policy governance contract gate | `node scripts/check-api-edge-security-policy-contract.mjs` 必須固定 gateway owner、API Gateway / WAF policy、OAuth2 / OIDC issuer、mTLS boundary、TLS termination owner、trusted header forwarding、identity propagation 與 evidence retention |
| SLO incident response governance contract gate | `node scripts/check-slo-incident-response-governance-contract.mjs` 必須固定 availability SLO、error budget policy、incident commander、escalation policy、mitigation decision、customer impact note 與 postmortem action owner |
| Structured log schema governance contract gate | `node scripts/check-structured-log-schema-governance-contract.mjs` 必須固定 `request_id`、`trace_id`、`severity`、`error_code`、`route`、log redaction policy、log retention owner 與 audit log review cadence |
| Secret handling governance contract gate | `node scripts/check-secret-handling-governance-contract.mjs` 必須固定 `API_KEY`、`PPROF_TOKEN`、bearer token file、secret mount、secret rotation owner、no hard-coded production credentials 與 incident artifact redaction |
| Supply chain artifact governance contract gate | `node scripts/check-supply-chain-artifact-governance-contract.mjs` 必須固定 SBOM、image signing、provenance / attestation、artifact retention、promotion approval 與 release evidence owner |
| Platform promotion policy contract gate | `node scripts/check-platform-promotion-policy-contract.mjs` 必須固定 platform promotion policy、environment approval、progressive rollout、platform-native signing、artifact verification 與 rollback owner |
| Deployment controller config contract gate | `node scripts/check-deployment-controller-config-contract.mjs` 必須固定 deployment controller、cloud environment template、environment manifest、progressive rollout controller、health gate、rollback trigger 與 promotion evidence |
...
| Trace shutdown contract | trace provider shutdown 必須使用 3 秒 bounded context，`api-worker` process exit 需呼叫 `obs.Shutdown(context.Background())` |
| Trace shutdown gate | `node scripts/check-trace-shutdown-contract.mjs` 必須固定 `TestTraceShutdownContract`、observability shutdown deadline、api-worker exit hook、README、章節、Makefile 與 CI 入口 |
| Prometheus config contract gate | `node scripts/check-prometheus-config-contract.mjs` 必須固定 scrape config、rule_files、alert rules、Compose monitoring profile、API key scrape auth 風險、Makefile 與 CI 入口 |
...
| Alertmanager routing governance contract gate | `node scripts/check-alertmanager-routing-contract.mjs` 必須固定 Alertmanager route、receiver owner、escalation owner、silence policy、notification evidence、Prometheus alerting target、Compose service、Makefile 與 CI 入口 |
| OTLP collector contract | `node scripts/check-otel-collector-contract.mjs` 必須固定 `production-api-worker/otel-collector.yaml`、OTLP gRPC receiver `0.0.0.0:4317`、`debug exporter`、Compose endpoint `OTEL_EXPORTER_OTLP_ENDPOINT=otel-collector:4317`、API contract scope、OpenAPI description、整合視覺課程、Makefile 與 CI 入口 |
| Operational runbook scope freshness contract gate | `node scripts/check-operational-runbook-scope-contract.mjs` 必須固定 runbook metadata、完整日期時間、API contract scope coverage、Docs publishing contract gate、Release artifact chain contract gate、Secret handling governance contract gate 與 Supply chain artifact governance contract gate |
| OTLP export governance contract gate | `node scripts/check-otel-export-governance-contract.mjs` 必須固定 local debug exporter、production backend owner、sampling rate、retention window、sensitive attribute redaction 與 trace data owner |
...
| Contract gate inventory | `node scripts/check-contract-gate-inventory-contract.mjs` 必須固定 58 個 root contract checker 都被 GitHub Actions 呼叫，且 Makefile、README、API contract、章節與整合視覺課程入口一致 |
...
| Go ReleaseNote contract gate | `node scripts/check-go-release-notes-contract.mjs` 必須固定 `scripts/generate-go-release-notes.mjs`、`ReleaseNote/`、`docs/ReleaseNote/`、Go 1.1-1.26 必要報告區塊、官方來源、支援狀態、Go 1.26.5 / Go 1.25.12 patch 訊號、Makefile 與 CI 入口 |
| Go ReleaseNote freshness evidence | `node scripts/check-go-release-notes-freshness-contract.mjs` 必須固定 official Go Release History verified 時間、Go 1.26.5 / Go 1.25.12 baseline、ReleaseNote index 來源證據、Makefile 與 CI 入口 |
| Release version consistency contract gate | `node scripts/check-release-version-consistency-contract.mjs` 必須固定 `VERSION`、`CHANGELOG.md`、README、API contract、OpenAPI、章節、整合視覺課程、`docs/index.html`、Makefile 與 CI 的目前版本一致性 |
| Release artifact chain contract gate | `node scripts/check-release-artifact-chain-contract.mjs` 必須固定同 timestamp 的審查報告、內容需要更新的部分、更新資料、VERSION、CHANGELOG、docs/index、Makefile 與 CI 入口 |
| Release artifact metadata consistency contract gate | `node scripts/check-release-artifact-metadata-contract.mjs` 必須固定同 timestamp 的審查報告、內容需要更新的部分與更新資料在版本、完整日期時間、本輪主題、來源引用與 artifact reference 上一致 |
| Release artifact continuity index contract gate | `node scripts/check-release-artifact-continuity-contract.mjs` 必須固定近期 release artifacts 的版本 sequence、timestamp 配對、三件套對應與 update record 自我引用 |
| Release publish reconciliation contract gate | `node scripts/check-release-publish-reconciliation-contract.mjs` 必須固定 remote release 已建立但 final release-record amend / tag 更新仍 local-only 時的 `HEAD`、`origin/main`、`tag^{}`、`force-with-lease`、recovery command，以及 blocked-push recovery 成功推送輸出與 Release publish recovery continuation |
| Dependency governance contract gate | `node scripts/check-dependency-governance-contract.mjs` 必須固定 root module 與 `production-api-worker` 的 `go mod verify`、`go list -m -u all`、`govulncheck ./...`、離線限制、Makefile 與 CI 入口 |
| Dependency audit evidence freshness contract gate | `node scripts/check-dependency-audit-evidence-contract.mjs` 必須固定 release evidence retention、root / production module dependency audit coverage，以及 module proxy / vulnerability database 離線時只能記錄為待補掃描 |
| Supply chain artifact governance contract gate | `node scripts/check-supply-chain-artifact-governance-contract.mjs` 必須固定 SBOM、image signing、provenance / attestation、artifact retention、promotion approval、release evidence owner、Makefile、CI、README、OpenAPI、章節與整合視覺課程入口 |
| Platform promotion policy contract gate | `node scripts/check-platform-promotion-policy-contract.mjs` 必須固定 platform promotion policy、environment approval、progressive rollout、platform-native signing、artifact verification、rollback owner、Makefile、CI、README、OpenAPI、runbook、章節與整合視覺課程入口 |
| Deployment controller config contract gate | `node scripts/check-deployment-controller-config-contract.mjs` 必須固定 deployment controller、cloud environment template、environment manifest、progressive rollout controller、health gate、rollback trigger、promotion evidence、Makefile、CI、README、OpenAPI、runbook、章節與整合視覺課程入口 |
| Performance benchmark governance contract | `node scripts/check-performance-benchmark-governance-contract.mjs` 必須固定 benchmark A/B、`benchstat old.txt new.txt`、pprof、metrics、Makefile、CI 與教材入口 |
| Release rollback drill contract | `node scripts/check-release-rollback-drill-contract.mjs` 必須固定 rollback decision、previous image restore、migration rollback boundary、health verification、metrics verification、postmortem evidence、Makefile 與 CI 入口 |
| Docker build contract | `node scripts/check-docker-build-contract.mjs` 必須固定 Dockerfile、`CGO_ENABLED=0`、`api-worker` / `migrate` binaries、`distroless/static-debian12` runtime image、`docker build -t production-api-worker:ci ./production-api-worker`、`docker build -t production-api-worker:standalone .`、Makefile 與 CI 入口 |
| Compose runtime env contract | `node scripts/check-compose-runtime-env-contract.mjs` 必須固定 `docker-compose.yml`、Postgres、migrate、api、OTEL collector、Prometheus `monitoring` profile、`DATABASE_URL`、`OTEL_EXPORTER_OTLP_ENDPOINT`、`API_KEY`、`REQUEST_BODY_LIMIT_BYTES`、`TRUSTED_PROXY_CIDRS`、`CORS_ALLOWED_ORIGINS`、service dependency、Makefile 與 CI 入口 |
...
| Worker shutdown | queue close 與 enqueue send 必須同步，shutdown 後新 enqueue 回穩定錯誤，並由 `node scripts/check-worker-shutdown-contract.mjs` 固定 |

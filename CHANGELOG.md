...
- 新增 `scripts/check-slo-incident-response-governance-contract.mjs` 與 `make slo-incident-response-governance-check`，固定 availability SLO、error budget policy、incident commander、escalation policy、mitigation decision、customer impact note 與 postmortem action owner。
- 同步 `README.md`、`production-api-worker/README.md`、`production-api-worker/docs/api-contract.md`、`production-api-worker/api/openapi.yaml`、operational runbook、第 7 / 9 / 11 章、進階 Cheat Sheet 與整合視覺課程，將本輪發版重點標示為 SLO incident response governance contract gate。
- Contract gate inventory 從 56 個 root contract checker 更新為 57 個 root contract checker，並納入 SLO incident response governance checker。
- `production-api-worker/api/openapi.yaml`、`production-api-worker/docs/api-contract.md` 與 version-sensitive contract checks 版本標記更新為 `v1.0.93`。
...
- 新增 `scripts/check-dependency-audit-evidence-contract.mjs` 與 `make dependency-audit-evidence-check`，固定 root module / `production-api-worker` 的 dependency audit evidence、`go mod verify`、`go list -m -u all`、`govulncheck ./...`、module proxy / vulnerability database 離線待補掃描語意與 release evidence retention。
- 同步 `README.md`、`production-api-worker/README.md`、`production-api-worker/docs/api-contract.md`、`production-api-worker/api/openapi.yaml`、operational runbook、第 7 / 9 / 11 章、進階 Cheat Sheet 與整合視覺課程，將本輪發版重點標示為 Dependency audit evidence freshness contract gate。
- Contract gate inventory 從 54 個 root contract checker 更新為 55 個 root contract checker，並納入 dependency audit evidence freshness checker。
- `production-api-worker/api/openapi.yaml`、`production-api-worker/docs/api-contract.md` 與 version-sensitive contract checks 版本標記更新為 `v1.0.91`。

## v1.0.90 - 2026-07-16

- 新增 2026-07-16 06:02:04 CST +0800 資深工程師審查報告，確認教程深度廣度已足夠，本輪缺口集中在 release artifacts 的歷史連續性尚未有獨立 gate 固定。
- 新增 `scripts/check-release-artifact-continuity-contract.mjs` 與 `make release-artifact-continuity-check`，固定近期 `審查報告/`、`內容需要更新的部分/`、`更新資料/` 的版本 sequence、timestamp 配對、三件套對應與 update record 自我引用。
- 同步 `README.md`、`production-api-worker/README.md`、`production-api-worker/docs/api-contract.md`、`production-api-worker/api/openapi.yaml`、第 7 / 9 / 11 章、進階 Cheat Sheet 與整合視覺課程，將本輪發版重點標示為 Release artifact continuity index contract gate。
- Contract gate inventory 從 53 個 root contract checker 更新為 54 個 root contract checker，並納入 release artifact continuity checker。
- `production-api-worker/api/openapi.yaml`、`production-api-worker/docs/api-contract.md` 與 version-sensitive contract checks 版本標記更新為 `v1.0.90`。
...
- 將分散在 API security、pprof diagnostics、Prometheus scrape auth 與 OTLP redaction 的 secret 管理邊界升級為正式 Secret handling governance contract gate，並同步 README、production README、operational runbook、API contract、OpenAPI、第 7 / 9 / 10 / 11 章、進階 Cheat Sheet 與整合視覺課程。
- Contract gate inventory 從 43 個 root contract checker 更新為 44 個，並納入 Secret handling governance contract checker。
- `production-api-worker/api/openapi.yaml`、`production-api-worker/docs/api-contract.md` 與 version-sensitive contract checks 版本標記更新為 `v1.0.75`。
...
- 新增 `scripts/check-compose-runtime-env-contract.mjs`，固定 `production-api-worker/docker-compose.yml` 的 Postgres、migrate、api、OTEL collector、Prometheus monitoring profile、runtime env、service dependency、Makefile、GitHub Actions 與教材入口。
- 將 Compose runtime env wiring 從既有 Docker Compose 設定升級為正式 Compose runtime env contract gate，並同步 README、production README、API contract、OpenAPI、第 7 / 9 / 11 章、進階 Cheat Sheet 與整合視覺課程。
- Contract gate inventory 從 40 個 root contract checker 更新為 41 個，並納入 Compose runtime env checker。
- `production-api-worker/api/openapi.yaml`、`production-api-worker/docs/api-contract.md` 與 version-sensitive contract checks 版本標記更新為 `v1.0.72`。
...
- 新增 `scripts/check-performance-benchmark-governance-contract.mjs`，固定 benchmark A/B、`benchstat old.txt new.txt`、pprof、metrics、Makefile、GitHub Actions 與教材入口。
- 將 Performance benchmark governance 從 README / 效能章節的建議升級為正式 contract gate，並同步 README、production README、API contract、OpenAPI、第 7 / 9 / 10 / 11 章、進階 Cheat Sheet 與整合視覺課程。
- Contract gate inventory 從 37 個 root contract checker 更新為 38 個，並納入 Performance benchmark governance checker。
- `production-api-worker/api/openapi.yaml`、`production-api-worker/docs/api-contract.md` 與 version-sensitive contract checks 版本標記更新為 `v1.0.69`。
...
- 新增 `scripts/check-ci-quality-gate-contract.mjs`，固定 README、production README、API contract、OpenAPI、第 7 / 9 / 11 章、進階 Cheat Sheet、整合視覺課程、Makefile 與 GitHub Actions 都保留 CI quality gate contract。
- `.github/workflows/ci.yml` 新增 `Check CI quality gate contract`；`production-api-worker/Makefile` 新增 `ci-quality-gate-check`。
- `production-api-worker/api/openapi.yaml`、`production-api-worker/docs/api-contract.md` 與既有 version-sensitive contract checks 版本標記更新為 `v1.0.53`。
- README、`production-api-worker/README.md`、第 7 / 9 / 11 章、進階 Cheat Sheet 與整合視覺課程同步加入 CI quality gate static gate。
- `docs/index.html` 已由整合課程重新同步並套用 GitHub Pages link fix。
...
- 新增 `scripts/check-trusted-proxy-contract.mjs`，固定 README、production README、API contract、operational runbook、config loader、config tests、rate limit key 實作、`TestRateLimitTrustedProxyContract`、第 7 / 11 章、進階 Cheat Sheet、整合視覺課程、Makefile 與 GitHub Actions 都保留 Trusted proxy client IP contract gate。
- `.github/workflows/ci.yml` 新增 `Check trusted proxy contract`；`production-api-worker/Makefile` 新增 `trusted-proxy-check`。
- `production-api-worker/api/openapi.yaml`、`production-api-worker/docs/api-contract.md` 與既有 version-sensitive contract checks 版本標記更新為 `v1.0.50`。
- README、`production-api-worker/README.md`、API contract、operational runbook、第 7 / 11 章、進階 Cheat Sheet 與整合視覺課程同步加入 Trusted proxy client IP contract gate。
